...
def parar_automacao():
    global executando
    executando = False
    salvar_progresso(linha_atual)  # salva a linha atual
    workbook.save('clientes_separados.xlsx')  # salva a planilha parcial
    label.config(text="Automação interrompida e planilha salva.")
...
def salvar_progresso(linha):
    with open("progresso.txt", "w") as f:
        f.write(str(linha))
        print("arquivo progresso.txt criado")
    ...
    try:
        WebDriverWait(navegador, tempo_max).until(
            EC.invisibility_of_element_located(
                (By.XPATH, '/html/body/app-root/app-carregando/div/mat-progress-spinner'))
        )
...
def processar_planilha():
    global executando, linha_atual

    for i in range(ultima_linha, len(DataFrame)):  # COMEÇA DA ÚLTIMA LINHA QUE FOI SALVA
        linha_atual = i  # Salva o índice atual em uma variável global
        ...
        if (cliente == "PETROLEO BRASILEIRO S A PETROBRAS" or cliente == "NACIOPETRO DISTRIB DE PETROLEO LTD" or
            ...
            for mtr_atual in mtrs_list:
                try:
                    ...
                    campo_mtr = WebDriverWait(navegador, 10).until(
                        EC.element_to_be_clickable((By.XPATH, "//input[@formcontrolname='manNumeroDestinador']"))
                        # XPATH do campo da MTR
                    ...
                    try:
                        ...
                        if (status_text == "Cancelado"
                            ...
                            try:
                                # Verifica quantas linhas estão presentes na tabela de tratamento
                                linhas_tabela = navegador.find_elements(By.XPATH, '//p-dialog[2]//table/tbody/tr')

                                if len(linhas_tabela) > 1:
                                    # Se houver mais de uma linha, cancela e salva o motivo

                                    sleep(1)

                                    navegador.find_element(By.XPATH,
                                                           '//p-dialog[2]//p-footer//button[span[text()="Cancelar"]]').click()

                                    motivo = "Dois ou mais campos para inserir o Tratamento"
                                    clientes_separados.append([ticket, cliente, mtr, peso, data, motivo])
                                    ...
                                    try:
                                        ...
                                        campo_peso = WebDriverWait(navegador, 10).until(
                                            EC.element_to_be_clickable(
                                                (By.XPATH, '//input[@formcontrolname="marQuantidadeRecebida"]'))
                                        )
                                        campo_peso.clear()
                                        ...
                                        motivo = "Tinha dois campos para inserir o tratamento"
